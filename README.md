...
- **Context Completeness is only moderate (r = 0.509), and part of that is a known bug.** `has_context` carries the largest weight in the dimension but fires on first-person *phrasing* rather than on content: "I need a function that sorts things" trips it and collects full marks, while an 86-word prompt that specifies types, edge cases and a signature without an "I am / I have" frame scores zero. The result is that we are too generous on contextless prompts and too stingy on well-specified ones, and on one benchmark pair we rank them in the opposite order to the judge. Fixing this does not require an LLM. What *does* remain out of reach is the deeper question: whether the context a prompt supplies is the context its task actually needs.
