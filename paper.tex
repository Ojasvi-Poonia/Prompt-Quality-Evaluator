...
One detail of the packaging carries an argument. The scorer's dependency list contains no LLM SDK of any kind; the judge's dependencies live in a separate \texttt{requirements-benchmark.txt}. So the central claim of this paper, that nothing neural runs at evaluation time, is not merely asserted in prose. It can be checked by reading what \texttt{pip} installs.
...
\subsection{Case 2: ``hello fix this ehing'' (terrible tier, with typo)}
This prompt is the reason the spell-checker exists at all. ``ehing'' is gibberish. An early build of our scorer happily treated it as a technical term, since the 1961 Brown corpus has no entry for ``ehing'' and an unknown word in the zipf-low band looked, to that build, like evidence of specificity. So a prompt that deserved Poor floated up to Below Average. A straight bug.

We fixed it by running pyspellchecker before specificity is counted. Now ``ehing'' is flagged as a likely typo with the suggestion ``thing'', and specificity drops $0.5$ rather than rising.

Final numbers: us 12.0, Gemini 0. Both Poor, both right. And ours says why, in the finding text: \emph{``Detected 1 likely typo: `ehing' $\to$ `thing'. Specificity reduced by 0.50.''} Gemini notes the typo somewhere in its rationale paragraph but never offers the correction.
...
This one pins down a single moderate dimension on its own. The prompt:

\emph{``I'm running Django 4.2 with PostgreSQL 14 and a Redis cache. We have a Task model with fields title (max 200), status (todo/in\_progress/done), priority (1--5), and due\_date. Write a ViewSet that supports CRUD with role-based permissions.''}

Fifty words, and exactly the context the task needs. We score Context Completeness at $0.50$, partly because the length factor nips a 50-word prompt slightly and partly because the CONTEXT discourse node is there but brief. Gemini gives $0.95$, reasoning that short as it is, the context fits.

The gap is a definition gap. We read context off surface markers: is there a CONTEXT node, how long is the prompt, how tight is the topic? Gemini reads semantic fit: does this context actually serve this task? ``Django 4.2 with PostgreSQL 14 and Redis'' is the right backdrop for a CRUD ViewSet only if you know what a ViewSet needs. Gemini knows. We do not.
...
\subsection{Failure mode 2: Context as semantic appropriateness}
Case 5 was the cleanest instance; here is another. \emph{``Write a SQL injection prevention function for a PHP application using PDO.''} Fourteen words. We put Context Completeness at $0.32$. Gemini puts it at $0.85$, because ``PHP application using PDO'' is exactly the context this task needs.

It is the Case 5 mismatch again: Gemini checking fit, us counting markers. The measured Pearson on this dimension, $0.509$, is only moderate, and the failure surfaces every time the prompt is short and its context is short but spot-on for the ask.

Two ways to close it without an LLM at scoring time. One, a labelled corpus to train a context-appropriateness classifier on top of the existing 41 features. Two, a retrieval setup matching the prompt against canonical task templates and the context each expects. Neither runs an LLM during evaluation. Both need data we have not collected yet.
...
Context Completeness ($r = 0.509$) is the cleanest warning sign. Anywhere a metric hinges on grasping what the task actually requires, or whether the material on the page serves that intent, we fall short. Tone sits in that bucket. So do genre conventions and domain norms. Surface features do not reach any of them.
...
Failure mode 2 showed Context Completeness turning on whether the context actually fits the task, which is a question our surface markers cannot answer. That is the single largest hole in what the pipeline can see.
